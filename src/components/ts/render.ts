...
function _render({ props, attrs, slots, slot, name }: { props: Record<string, any>; slot: Record<string, any>; attrs: Record<string, any>; slots: Slots; name: string }) {
            ...
            if (!isValidElement(firstChild) || other.length > 0) {
                throw new Error(
                    [
                        ...
                        ['Add an `as="..."` prop, to ensure that we render an actual element instead of a "template".', "Render a single element as the child so that we can forward the props onto that element."]
                            .map((line) => `  - ${line}`)
                            .join("\n"),
                    ].join("\n")
                );
